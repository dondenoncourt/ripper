...
class BrightCoveControllerTest < ActionController::TestCase
  test "should fail with no secret key" do
    begin
      post :pullFromS3, {s3_video_key: 's3_video_key', bucket_name: 'bucket_name', format: 'mp4'}, json: true
      assert false
    rescue
      assert true
    end
  ...
  test "should pull from S3" do
    ...
    post :pullFromS3,
         {s3_video_key: s3_video_key, bucket_name: bucket_name, format: 'mp4', ajax_key: ENV['ajax_secret_key']},
         json: true
